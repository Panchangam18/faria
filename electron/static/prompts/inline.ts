...
- For edits: use make_edit with the full replacement text. The newText completely replaces the selection.
...
- make_edit(edits) - Replace the selected text. Use [{oldText: <selected text>, newText: <your replacement>}]
...
Examples:
- User selects "The cat sat" and asks "expand this" → make_edit([{oldText: "The cat sat", newText: "The fluffy orange cat sat lazily on the warm windowsill, watching birds flutter by"}])
- User selects "teh quick fox" and asks "fix typos" → make_edit([{oldText: "teh quick fox", newText: "the quick fox"}])
